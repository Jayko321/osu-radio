...
use super::types::parse_lazer_beatmap_line;

#[test]
fn parses_lazer_beatmap_json_into_core_type() {
    let beatmap = parse_lazer_beatmap_line(
        r#"{"source":"Lazer","difficulty_name":"Hard","bpm":180.5,"hash":"beatmap-hash","metadata":{"title":"Song","title_unicode":"Song Unicode","artist":"Artist","artist_unicode":"Artist Unicode","author":{"online_id":456,"username":"Mapper","country_code":"EE"},"source":"Game","tags":"tag one","user_tags":["favorite"],"preview_time":12345,"audio_file":"audio.mp3","background_file":"bg.jpg"},"beatmap_set":{"online_id":123,"hash":"set-hash","files":[{"filename":"audio.mp3","file":{"hash":"abc","resolved_path":"C:\\osu\\files\\a\\ab\\abc"}}]}}"#,
    )
    .unwrap();

    assert_eq!(beatmap.source, OsuKind::Lazer);
    ...
    let beatmap_set = beatmap.beatmap_set.unwrap();
...
mod helper_process {
    ...
    async fn accepts_a_fake_ndjson_helper() {
        let helper = fake_helper(
            "printf '%s\\n' '{\"source\":\"Lazer\",\"metadata\":{\"title\":\"test\"}}'",
        );
