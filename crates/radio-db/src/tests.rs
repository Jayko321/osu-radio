...
use crate::{
    Database, ImportSummary,
    entities::{audio_source, beatmap, beatmap_metadata, beatmap_set},
    metadata_hash,
    model::{OsuInstallationChanges, SourceType},
};
...
async fn register(database: &Database, name: &str) -> i32 {
...
async fn registrations_and_updates(database: &Database, other_pool: &Database) {
    ...
    assert_eq!(
        database
            .osu_installations()
            .update(original.id, OsuInstallationChanges::default())
            .await
            .unwrap(),
        Some(original.clone())
    );
    ...
    assert!(
        database
            .osu_installations()
            .update(i32::MAX, OsuInstallationChanges::default())
            .await
            .unwrap()
            .is_none()
    );
...
#[allow(clippy::too_many_lines)]
async fn snapshots_and_cleanup(database: &Database, other_pool: &Database) {
    ...
    // Both shared foreign keys reject deletion while beatmaps reference them.
    assert!(
        beatmap_metadata::Entity::delete_by_id(shared_hash.clone())
            .exec(&database.connection)
            .await
            .is_err()
    );
    assert!(
        audio_source::Entity::delete_by_id(first_audio)
            .exec(&database.connection)
            .await
            .is_err()
    );
    ...
    assert!(grouped.iter().all(|(_, sources)| sources.len() == 1));
    ...
    let maps = database.beatmaps().for_set(grouped[0].0.id).await.unwrap();
...
async fn rollback_preserves_snapshot(database: &Database) {
...
#[cfg(feature = "sqlite")]
async fn install_failure_trigger(database: &Database) {
...
async fn remove_failure_trigger(database: &Database) {
    #[cfg(feature = "sqlite")]
...
async fn concurrent_replacements_and_deletion(database: &Database, other_pool: &Database) {
...
async fn shared_source_variants(database: &Database) {
    let mut ids = Vec::new();
    for source in [
...
async fn metadata_corruption_is_rejected(database: &Database) {
    ...
    let installation = register(database, "cleanup-corrupted").await;
    database
        .osu_installations()
        .delete(installation)
        .await
...
async fn source_files_remain_untouched(database: &Database) {
    let directory = tempfile::tempdir().unwrap();
