...
2. Read only the relevant component guide: [scanner](scanner.md),
   [frontend](frontend.md), or [backend](backend.md). Follow its source/test links
...
| `osu-radio-server` | [Startup](../../apps/osu-radio-server/src/main.rs), [routes](../../apps/osu-radio-server/src/routes/mod.rs), [services](../../apps/osu-radio-server/src/services/mod.rs) | Changing backend orchestration, HTTP translation or hosting. Database-dependent contracts are deferred. |
| `radio-db` | [Package boundary](../../crates/radio-db/Cargo.toml) | Current rewrite priority; internals and workflows are intentionally outside this documentation. |
...
flowchart LR
    ...
    Server --> DB["radio-db: rewrite deferred here"]
...
This diagram deliberately omits persistence workflows. The backend boundary owns
...
| Client session, child supervision, HTTP wrappers | Implemented | [Frontend](frontend.md), [backend](backend.md); readiness output is a protocol, API contracts are deferred. |
...
| Database behavior and dependent API contracts | Deferred from documentation | Existing code is under redesign; this is not a claim that those components are absent. |
...
The immediate priority is the `radio-db` rewrite. Database internals,
database-backed API contracts and persistence workflows are deliberately
excluded from all these guides and shared entry points. Hosting, providers,
mobile implementation, playback implementation and the redesigned persistence
contracts are not selected here. Defer the server/client API skill until those
contracts settle; do not fill gaps with speculative architecture.
