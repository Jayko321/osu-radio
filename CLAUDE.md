...
The desktop GUI sits on the other side of that HTTP boundary and is deliberately split in two. `crates/osu-radio-client` is the framework-agnostic half: it supervises the server process, wraps every endpoint, owns its own DTOs, and carries the small pieces of UI state any frontend needs (`Loading<T>`, `describe`, and the view models under `src/view_models/`). `apps/osu-radio-gui-vizia` is the vizia half and holds nothing but signals, views, and events. The split is the point: a second frontend should add an app next to it and reimplement views alone, so a GUI toolkit must never reach the client crate.

Its two screens — a songs library and a settings pane, both over a shared now-playing stage — are built to the Figma design, and they run on placeholder content in `src/sample.rs` rather than on anything the server returns. That is deliberate and temporary: the shape of the UI was worth settling before the read path behind it grew. Wiring it up means filling `sample::TRACKS` — already a `Vec<osu_radio_client::Track>` — from beatmap sets fetched through `ApiClient`, which is what `src/views/` is written to make easy — every component is a free function taking the data it renders as a parameter, so the swap happens at the call site rather than inside the view. The look lives entirely in `styles/`, split one sheet per component and loaded by `views::styles`, so layout and colour changes need no recompile in a debug build.
